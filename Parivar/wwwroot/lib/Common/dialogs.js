var Parivar = new function () {
    ...
    this.SuccessOk = function (title = "", dialogMsg, yesCallback) {
        title === "" ? "Success" : title;
        try {
            swal({ title: title, text: dialogMsg, icon: this.typeModel.Success, closeOnConfirm: false }).then((isConfirmed) => {
                if (isConfirmed) {
                    yesCallback();
                }
            });
    ...
    this.Success = function (title = "", dialogMsg) {
        title === "" ? "Success" : title;
        try {
            swal({ title: title, text: dialogMsg, icon: this.typeModel.Success, timer: 2000, showConfirmButton: true });
        } catch (err) {
            alert(dialogMsg);
        }
    };
