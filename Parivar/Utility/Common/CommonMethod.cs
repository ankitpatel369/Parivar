...
namespace Parivar.Utility.Common
{
    public static class CommonMethod
    {
        public static List<string> GetModelError(this ModelStateDictionary model)
        {
            return model.Values.SelectMany(m => m.Errors)
        ...
        /// <summary>
        /// Convert to Datatable type Object from List of T types item
        /// </summary>
        /// <typeparam name="T">Type of List</typeparam>
        /// <param name="items">List of T type items</param>
        /// <returns>Return Datatable Object from List of T Types item</returns>
        ...
        /// <summary>
        /// Compare First date with second date and returns false if first date is after second date
        /// </summary>
        ...
        public static bool IsValidStartEndDate(DateTime startDate, DateTime endDate)
        {
            try
            {
                var result = DateTime.Compare(startDate.Date, endDate.Date);
        ...
        private static readonly byte[] SaltBytes = { 0x49, 0x76, 0x61, 0x6e, 0x20, 0x4d, 0x65, 0x64, 0x76, 0x65, 0x64, 0x65, 0x76 };

        public static byte[] EncryptFile(byte[] plainBytes, string encryptionKey)
        {
            byte[] encryptedBytes = null;

            // create a key from the password and salt,
            var password = new Rfc2898DeriveBytes(encryptionKey, SaltBytes, 32768);
        ...
        public static byte[] DecryptFile(byte[] cipherBytes, string encryptionKey)
        {
        ...
        public static string ReadEmailTemplate(IErrorLogService logService, string currentPath, string templateName, string physicalUrl)
        {
            try
            {
                string fullPath = CheckServerPath(currentPath, @"EmailTemplate\", templateName);
                string body;
        ...
        public static string CreateRandomPasswordWithRandomLength()
        {
            ...
            var size = random.Next(8, validChars.Length);
        ...
        public static string CreateRandomStringByLength(int length = 5)
        {
        ...
        public static JQueryDataTableSqlParam GetJQueryDatatableParamList(JQueryDataTableParamModel param, long timeZoneId, string sortingColumn)
        {
            var searchRecords = new SqlParameter { ParameterName = "@SearchRecords", SqlDbType = SqlDbType.Int, Direction = ParameterDirection.Output };
            var parameters = new JQueryDataTableSqlParam()
            {
                ...
                Parameters = new List<SqlParameter>
                {
                    new SqlParameter("@TimeZoneId",SqlDbType.Int){Value = timeZoneId},
                    new SqlParameter("@iDisplayStart",SqlDbType.Int){Value = param.iDisplayStart},
                    new SqlParameter("@iDisplayLength",SqlDbType.Int){Value = param.iDisplayLength},
                    new SqlParameter("@SortColumn",SqlDbType.VarChar){Value = sortingColumn},
                    new SqlParameter("@SortDir",SqlDbType.VarChar){Value = param.sSortDir_0 ?? ""},
